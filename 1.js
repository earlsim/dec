...
/* harmony import */ var vivus__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! vivus */ "./node_modules/vivus/dist/vivus.js");
...
/* harmony import */ var _node_modules_flatpickr_dist_l10n_ru_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../../node_modules/flatpickr/dist/l10n/ru.js */ "./node_modules/flatpickr/dist/l10n/ru.js");
/* harmony import */ var _node_modules_flatpickr_dist_l10n_ru_js__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_node_modules_flatpickr_dist_l10n_ru_js__WEBPACK_IMPORTED_MODULE_4__);
/* harmony import */ var _modules_Range__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./modules/Range */ "./src/js/modules/Range.js");
...
window.contentHasLoaded = false; // Индикатор прорисовки прелоадера
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var preLoaderElement = window.preLoaderElement = document.querySelector('.js-pre-loader') || false;

  if (preLoaderElement) {
    var svgPreLoader = window.svgPreLoader = new vivus__WEBPACK_IMPORTED_MODULE_2___default.a('svg-pre-loader', {
      type: 'scenario'
    });
...
//  new _modules_Select__WEBPACK_IMPORTED_MODULE_6__["default"](".js-field-select"); // Range цены в каталоге

  new _modules_Range__WEBPACK_IMPORTED_MODULE_5__["default"]("field-range"); // Конфигурация модальных окон

  _modules_modal_js__WEBPACK_IMPORTED_MODULE_7__["default"].init({
    disableScroll: true
  }); // Head slider element

  var headSliderNode = document.querySelector('#head-slider') || false; // Header

  var header = window.header = document.querySelector('header.header') || false; // Header Slider
  // Слайдер в шапке на главной

  new swiper__WEBPACK_IMPORTED_MODULE_1__["default"]('.js-head-slider', {
    speed: 1000,
    effect: 'fade',
    pagination: {
  ...
  new swiper__WEBPACK_IMPORTED_MODULE_1__["default"]('.js-inst-slider', {
    speed: 400,
    spaceBetween: 24,
    slidesPerView: 4,
    autoplay: {
  ...
  if (headSliderNode) {
    ...
    var initialPoint;
    var finalPoint;
    document.querySelector('#head-slider').addEventListener('touchstart', function (e) {
      e.preventDefault();
      e.stopPropagation();
      initialPoint = e.changedTouches[0];
    });
    document.querySelector('#head-slider').addEventListener('touchend', function (e) {
      e.preventDefault();
      e.stopPropagation();
  ...
  formQuickOrderSend(); // Форма отправки обратной связи
  ...
  checkoutStepFirst();
  checkoutStepSecond();
  checkoutStepFourth(); // checkoutFormSend();
  //

  deleteProductFromCart(); // Header menu btn click
  // Кнопка меню в хедере при клике

  var menuBtn = window.menuBtn = document.querySelector('.js-menu-btn') || false;
  var menuContainer = window.menuContainer = document.querySelector('.js-menu-container') || false;
  ...
  var headerFlowersCloseBtn = document.querySelector('.js-header-flowers-close') || false;
  ...
  if (headerFlowersCloseBtn) {
    headerFlowersCloseBtn.addEventListener('click', function (e) {
      ...
      if (document.body.clientWidth < 1200) {
        ...
        var headerFlowersContainer = this.parentNode.parentNode;
  ...
  [].slice.call(document.querySelectorAll('.js-tab-btn')).forEach(function (item) {
    "use strict";

    item.addEventListener('click', function (e) {
      e.preventDefault();
      var id = parseInt(this.getAttribute('data-id'));
      [].slice.call(document.querySelectorAll('[data-tab-bouquets].is-active')).forEach(function (item) {
        item.classList.remove('is-active');
      });

      if (id > 0) {
        [].slice.call(document.querySelectorAll("[data-tab-bouquets=\"".concat(id, "\"]"))).forEach(function (item) {
          item.classList.add('is-active');
        });
      } else {
        [].slice.call(document.querySelectorAll("[data-tab-bouquets]")).forEach(function (item, id) {
          if (id < 6) {
            item.classList.add('is-active');
          }
        });
      ...
      [].slice.call(document.querySelectorAll('.js-tab-btn.is-active')).forEach(function (item) {
        item.classList.remove('is-active');
      });
  ...
  [].slice.call(document.querySelectorAll('.js-filter-apply')).forEach(function (item) {
    "use strict";
  ...
  $('.js-showmore').click(function () {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get(location.pathname, $form.serialize() + '&page=' + nextPage, function (response) {
  ...
  $(document).on('click', '.js-product-qnt [data-action]', function (e) {
    e.preventDefault();
    var inputElement = this.parentNode.querySelector('input[type=text]');
    var val = parseInt(inputElement.value);
    var actualPrice = parseInt(this.parentNode.parentNode.parentNode.dataset.price);

    if (!actualPrice) {
      actualPrice = parseInt(this.parentNode.parentNode.parentNode.parentNode.dataset.price);
    }

    var length = parseInt($('div[data-length-active]').attr('data-price'));

    if (isNaN(length)) {
      length = 0;
    }

    var color = parseInt($('div[data-color-active]').attr('data-price'));
    ...
    if (!actualPrice) {
      actualPrice = parseInt($('.js-result-price').attr('data-new-price')) + color + length;
    }

    ;
    var priceElem = this.parentNode.parentNode.querySelector('.product-card__price__value');
    ...
    var sum = parseInt(priceElem.innerHTML);
    ...
    if (this.getAttribute('data-action') === 'min') {
      ...
      if (document.querySelector('.js-product-qnt').hasAttribute('data-construct')) {
        ...
        if (val == 20) {
          ...
          var _currentImg = document.querySelector('.result-pic__img');

          var _nextImg = _currentImg.getAttribute('data-color-' + colorCount + '-second-src');

          _currentImg.setAttribute('src', _nextImg);
        } else if (val == 100) {
          var _currentImg2 = document.querySelector('.result-pic__img');

          var _nextImg2 = _currentImg2.getAttribute('data-color-' + colorCount + '-third-src');

          _currentImg2.setAttribute('src', _nextImg2);
        }
      }
      ...
      if (document.querySelector('.js-product-qnt').hasAttribute('data-construct')) {
        ...
        var _colorCount = $('.js-color-toggle[data-color-active]').attr('data-color-count');

        if (!_colorCount) {
          _colorCount = $('.js-color-toggle').attr('data-color-count');
        }

        if (val == 21) {
          var _currentImg3 = document.querySelector('.result-pic__img');

          var _nextImg3 = _currentImg3.getAttribute('data-color-' + _colorCount + '-second-src');

          _currentImg3.setAttribute('src', _nextImg3);
        } else if (val == 51) {
          var _currentImg4 = document.querySelector('.result-pic__img');

          var _nextImg4 = _currentImg4.getAttribute('data-color-' + _colorCount + '-third-src');

          _currentImg4.setAttribute('src', _nextImg4);
        } else if (val == 101) {
          var _currentImg5 = document.querySelector('.result-pic__img');

          var _nextImg5 = _currentImg5.getAttribute('data-color-' + _colorCount + '-fourth-src');

          _currentImg5.setAttribute('src', _nextImg5);
        }
      }
    ...
    inputElement.value = val; // $('.result-info__qty').hide().html('');
  ...
  [].slice.call(document.querySelectorAll('.js-product-cart-qnt [data-action]')).forEach(function (item) {
    "use strict";
    ...
    item.addEventListener('click', function (e) {
      ...
      var inputElement = this.parentNode.querySelector('input[type=text]');
      var val = parseInt(inputElement.value);
      var price = parseInt($(this).closest('.js-product-cart-item').find('.js-price-val').html().replace(/\s/g, ''));
      ...
      var id = parseInt(this.parentNode.parentNode.getAttribute("data-cart-prod"));
      ...
      timer = setTimeout(function () {
        jquery__WEBPACK_IMPORTED_MODULE_0___default.a.post('/cart/save/', {
          html: 1,
          id: id,
          quantity: actQty
        }, function (response) {
          if (response.status == 'ok') {
            ...
            $this.closest('.js-product-cart-item').find('.js-sum-val').html(itemSum); // that.val(response.data.q);
          }
  ...
  $(".js-cart-product-delete").click(function () {
    ...
    var cartQty = $('.js-header-cart-count');
    jquery__WEBPACK_IMPORTED_MODULE_0___default.a.post('/cart/delete/', {
      html: 1,
      id: id
    }, function (response) {
      cartSetQuantity(response.data.count);
  ...
  //   }, false);
  // });
  //Склонение окончаний в конструкторе
  ...
  $(document).on('input', '.js-product-qnt [type=text]', function (e) {
    ...
    if (!actualPrice) {
      actualPrice = parseInt(document.querySelector('.js-result-price').dataset.price) + length + color;
    }
    ...
    if (document.querySelector('.js-product-qnt').hasAttribute('data-construct')) {
      var _inputElement = document.querySelector('.js-product-qnt [type=text]');
      ...
      var _val = parseInt(_inputElement.value);
      ...
      _inputElement.value = parseInt(_val, 10);

      if (isNaN(sum)) {
        priceElem.innerHTML = '0';
      } else {
        priceElem.innerHTML = sum;
        $('.result-info__qty').html(parseInt(_val, 10));
      }
      ...
      if (_val >= 0 && _val < 21) {
        ...
        currentImg.setAttribute('src', nextImg);
        console.log(nextImg);
      } else if (_val >= 21 && _val < 51) {
        var _currentImg6 = document.querySelector('.result-pic__img');

        var _nextImg6 = _currentImg6.getAttribute('data-color-' + colorCount + '-second-src');

        _currentImg6.setAttribute('src', _nextImg6);
      } else if (_val >= 51 && _val < 101) {
        var _currentImg7 = document.querySelector('.result-pic__img');

        var _nextImg7 = _currentImg7.getAttribute('data-color-' + colorCount + '-third-src');

        _currentImg7.setAttribute('src', _nextImg7);
      } else if (_val >= 101) {
        var _currentImg8 = document.querySelector('.result-pic__img');

        var _nextImg8 = _currentImg8.getAttribute('data-color-' + colorCount + '-fourth-src');

        _currentImg8.setAttribute('src', _nextImg8);

        console.log(_nextImg8);
      }
    }
  }); // Кнопка отображения списка цветов в хедере

  var headerMenuFlowersBtn = document.querySelector('.js-header-menu-flowers-btn') || false;

  if (headerMenuFlowersBtn) {
    headerMenuFlowersBtn.addEventListener('click', function (e) {
      e.preventDefault();
      var headerFlowersContainer = document.querySelector('.header-flowers-container');
  ...
  [].slice.call(document.querySelectorAll('.js-header-menu-icon')).forEach(function (item) {
    item.addEventListener('click', function (e) {
      e.preventDefault();
      console.log(this);
    }, false);
  ...
  var filterBtnList = document.querySelectorAll('.js-filter-btn');
  [].slice.call(filterBtnList).forEach(function (item) {
    "use strict";
    ...
    item.addEventListener('click', function (e) {
      ...
      e.preventDefault();
      [].slice.call(filterBtnList).forEach(function (item) {
        if (item !== _this && item.parentNode.classList.contains('is-active')) {
  ...
  var filterBtnResetList = document.querySelectorAll('.js-filter-reset');
  [].slice.call(filterBtnResetList).forEach(function (item) {
    "use strict";
  ...
  var checkoutToggleList = document.querySelectorAll('.js-checkout-toggle');
  [].slice.call(checkoutToggleList).forEach(function (item) {
    "use strict";
    ...
    item.addEventListener('click', function (e) {
      ...
      var id = Number(e.currentTarget.querySelector('.checkout-step__title').getAttribute('data-id')); // console.log(id);
  ...
  var checkoutBtnList = document.querySelectorAll('.js-checkout-btn');
  [].slice.call(checkoutBtnList).forEach(function (item) {
    "use strict";

    item.addEventListener('click', function (e) {
      e.preventDefault();
      var id = parseInt(this.getAttribute('data-checkout-step'));
      var fields = document.querySelectorAll("[data-validate-step=\"".concat(id, "\"]"));
      var form = new FormData(document.getElementById('checkout'));
      /*
      fetch('response.php', {
        headers: {
      ...
      */
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default.a.ajax({
        url: "/response.php?test=step-".concat(id),
        ...
        success: function success(res) {
          // console.log(res);
          res = JSON.parse(res);
          var stepNext = document.querySelector("[data-id=\"0".concat(id + 1, "\"]")) || false;

          if (stepNext) {
            document.querySelector("[data-id=\"0".concat(id, "\"]")).parentNode.parentNode.classList.remove('is-active');
            document.querySelector("[data-id=\"0".concat(id, "\"]")).parentNode.parentNode.classList.add('is-success');
            setTimeout(function () {
        ...
        statusCode: {
          422: function _(res) {
            // console.log(res);
            res = res.responseJSON; // res = JSON.parse(res);

            for (var name in res.errors) {
              if (res.errors.hasOwnProperty(name)) {
                // console.log(`input[name="${name}"]`);
                document.querySelector("input[name=\"".concat(name, "\"]")).parentNode.classList.add('has-error');
              }
            }
          }
        }
  ...
  var modalFeedbackBtns = document.querySelectorAll('.js-modal-feedback-btn') || false;

  if (modalFeedbackBtns.length > 0) {
    [].slice.call(modalFeedbackBtns).forEach(function (item) {
      "use strict";

      item.addEventListener('click', function (e) {
        e.preventDefault();
        _modules_modal_js__WEBPACK_IMPORTED_MODULE_7__["default"].show('modal-feedback');
      });
    });
  }

  var modalWeddingBtns = document.querySelectorAll('.js-modal-wedding-btn') || false;

  if (modalWeddingBtns.length > 0) {
    [].slice.call(modalWeddingBtns).forEach(function (item) {
      "use strict";

      item.addEventListener('click', function (e) {
        e.preventDefault();
        _modules_modal_js__WEBPACK_IMPORTED_MODULE_7__["default"].show('modal-wedding');
      });
    });
  } // Modals


  var modalOrderBtn = document.querySelectorAll('.js-modal-order-btn') || false;

  if (modalOrderBtn.length > 0) {
    modalOrderBtn.addEventListener('click', function (e) {
      e.preventDefault(); // VModal.show('modal-order');
    });
  } // Header shopping cart


  var headerCartBtn = document.querySelector('.js-header-cart') || false;
  ...
  var headerFlowersLink = document.querySelectorAll('.js-header-flowers-link');
  [].slice.call(headerFlowersLink).forEach(function (item) {
    "use strict";
  ...
  var filtersBtn = document.querySelector('.js-filters-btn') || false;
  ...
  $(document).on('click', ".js-product-to-favorites", function (e) {
    e.preventDefault();
    var productCardElement = this.parentNode;
    var productId = productCardElement.getAttribute('data-id');
    jquery__WEBPACK_IMPORTED_MODULE_0___default.a.post("/wishlist/toggle/", {
      html: 1,
  ...
  [].slice.call(document.querySelectorAll('.js-product-like')).forEach(function (item) {
    "use strict";
  ...
  [].slice.call(document.querySelectorAll('.js-favorites-delete')).forEach(function (item) {
    "use strict";

    item.addEventListener('click', function (e) {
      e.preventDefault();
      var productCardElement = this.parentNode;
      var productId = productCardElement.getAttribute('data-id'); // console.log(favoritesCount);
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default.a.post("/wishlist/toggle/", {
        ...
        if (res.status == 'ok') {
          ...
          var count = res.data.count; // favoritesSetQuantity(res.data.quantity);

          $('.page__title--data').attr('data-content', count); // favoritesCount.val(res.data.count);

          if (res.data.count === 0) {
            var favoritesPage = document.querySelector("#favorites-page") || false;
  ...
  */
}, false); // Загрузка лоадера и контента отображение контента после лоадера
...
function filters() {
  var params = [];
  var flowers = document.querySelectorAll('[data-filter-flower] input[type=checkbox]:checked');
  var colors = document.querySelectorAll('[data-filter-color] input[type=checkbox]:checked');
  var priceMin = parseInt(document.querySelector('[data-filter-price-min] input[type=text]').value);
  var priceMax = parseInt(document.querySelector('[data-filter-price-max] input[type=text]').value);
  var boxes = document.querySelectorAll('[data-filter-box] input[type=checkbox]:checked');

  if (flowers.length > 0) {
    params[params.length] = [];
    [].slice.call(flowers).forEach(function (item) {
      "use strict";
  ...
  if (colors.length > 0) {
    params[params.length] = [];
    [].slice.call(colors).forEach(function (item) {
      "use strict";
  ...
  if (boxes.length > 0) {
    params[params.length] = [];
    [].slice.call(boxes).forEach(function (item) {
      "use strict";
  ...
  if (priceMin > 0) {
    params[params.length] = "price_min=".concat(priceMin);
  }

  params[params.length] = "price_max=".concat(priceMax);
  window.location.search = "?" + params.join('&');
}

function filtersWithoutFlowers() {
  var params = []; // let flowers = document.querySelectorAll('[data-filter-flower] input[type=checkbox]:checked');
  ...
  if (priceMin > 0) {
    params[params.length] = "min=".concat(priceMin);
  }

  params[params.length] = "max=".concat(priceMax);
  window.location.search = "?" + params.join('&');
}

function filtersWithoutColors() {
  var params = [];
  var flowers = document.querySelectorAll('[data-filter-flower] input[type=checkbox]:checked'); // let colors = document.querySelectorAll('[data-filter-color] input[type=checkbox]:checked');
...
function filtersWithoutPrice() {
  var params = [];
  var flowers = document.querySelectorAll('[data-filter-flower] input[type=checkbox]:checked');
  var colors = document.querySelectorAll('[data-filter-color] input[type=checkbox]:checked'); // let priceMin = parseInt(document.querySelector('[data-filter-price-min] input[type=text]').value);
...
function filtersWithoutBoxes() {
  ...
  var colors = document.querySelectorAll('[data-filter-color] input[type=checkbox]:checked');
  var priceMin = parseInt(document.querySelector('[data-filter-price-min] input[type=text]').value);
  var priceMax = parseInt(document.querySelector('[data-filter-price-max] input[type=text]').value); // let boxes = document.querySelectorAll('[data-filter-box] input[type=checkbox]:checked');
...
$('body').on('submit', '.js-login-form', function (e) {
  e.preventDefault();
  var $form = $(this);
  jquery__WEBPACK_IMPORTED_MODULE_0___default.a.post('/login/', $form.serialize(), function (response) {
    if (response.status == 'fail') {
      if (response.errors.login) {
        $form.find('.js-field-wrap').addClass('is-unconfirmed').find('.js-field-msg').html(response.errors.login.confirm_email);
      } else {
        $form.find('.js-field-wrap').addClass('has-error').find('.js-field-msg').addClass('is-active').html(response.errors.auth);
      }
    } else {
      location.reload();
    }
...
$('body').on('submit', '.js-signup-form', function (e) {
  e.preventDefault();
  var $form = $(this);
  jquery__WEBPACK_IMPORTED_MODULE_0___default.a.post('/signup/', $form.serialize(), function (response) {
    $form.find('.js-field-msg-pass').empty();
    $form.find('.js-field-msg').empty();

    if (response.status == 'fail') {
      jquery__WEBPACK_IMPORTED_MODULE_0___default.a.each(response.errors, function (key, val) {
        ...
        // }
          ...
          $form.find('.js-field-msg').html(response.errors.email.exists);
        }
      });
      $('html, body').animate({
...
function checkoutStepFirst() {
  ...
  var fieldRegisterMe = document.querySelector('input[name="register_me"]') || false;
...
function checkoutStepSecond() {
  var items = document.querySelectorAll('input[name="street"], input[name="building"], input[name="flat"], input[name="date"]');
  var message = "";
  [].slice.call(items).forEach(function (item) {
    item.addEventListener('change', function (e) {
      message = "\u0414\u043E\u0441\u0442\u0430\u0432\u043A\u0430 \u043F\u043E \u0430\u0434\u0440\u0435\u0441\u0443: [\u0433. \u041F\u044F\u0442\u0438\u0433\u043E\u0440\u0441\u043A], \u0443\u043B. ".concat(items[0].value, ", \u0434. ").concat(items[1].value, ", \u043A\u0432. ").concat(items[2].value, ", ").concat(items[3].value);
...
function checkoutStepFourth() {
  var fieldRecipient = document.querySelector('input[name="recipient"]') || false;
  ...
  var fieldMessage = document.querySelector('input[name="message"]') || false;
...
function createBlockCartEmptyHeader() {
  ...
  div.classList.add('header-cart-container__empty');
  div.innerText = "Корзина пуста";
  var container = document.querySelector('js-header-cart-container');
  container.innerHTML = "";
  container.append(div);
}

function createBlockCartEmptyPage() {
  var shoppingCartPage = document.querySelector("#shopping-cart-page") || false;
...
function createBlockCheckoutEmptyPage() {
  var checkoutPage = document.querySelector("#checkout-page") || false;
...
function formFeedBackSend() {
  ...
  if (feedBack) {
    feedBack.addEventListener('submit', function (e) {
      e.preventDefault(); // Сбрасываем предыдущие ошибки полей перед запросом
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default.a.ajax({
        ...
        success: function success(res) {
          ...
          // res = JSON.parse(res);
          if (res.status == 'ok') {
            _modules_modal_js__WEBPACK_IMPORTED_MODULE_7__["default"].close('modal-feedback');
            document.querySelector('#modal-message-text').innerText = res.data.html;
            _modules_modal_js__WEBPACK_IMPORTED_MODULE_7__["default"].show('modal-message');
          }
        },
        statusCode: {
          422: function _(err) {
...
function formWeddingSend() {
  var wedding = document.querySelector('#wedding') || false;
    ...
    wedding.addEventListener('submit', function (e) {
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default.a.ajax({
        ...
        success: function success(res) {
          ...
          // res = JSON.parse(res);
          if (res.status == 'ok') {
            _modules_modal_js__WEBPACK_IMPORTED_MODULE_7__["default"].close('modal-wedding');
            document.querySelector('#modal-message-text').innerText = res.data.html;
            _modules_modal_js__WEBPACK_IMPORTED_MODULE_7__["default"].show('modal-message');
          }
        },
...
function formOrderSend() {
  var feedBack = document.querySelector('#orderForm') || false;
...
function formQuestionSend() {
  var question = document.querySelector('#questionForm') || false;
...
function formQuickOrderSend() {
  var form = document.querySelector('#order-quick') || false;
    ...
    form.addEventListener('submit', function (e) {
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default.a.ajax({
        ...
        success: function success(res) {
          ...
          // console.log(res);
          if (res.success) {
            _modules_modal_js__WEBPACK_IMPORTED_MODULE_7__["default"].close('modal-order');
            document.querySelector('#modal-message-text').innerText = res.message;
            _modules_modal_js__WEBPACK_IMPORTED_MODULE_7__["default"].show('modal-message');
          }
        },
...
function deleteProductFromCart() {
  [].slice.call(document.querySelectorAll('[data-cart-prod-delete]')).forEach(function (item) {
    item.addEventListener('click', dPFC);
  });
}

function dPFC(e) {
  var productNode = e.currentTarget.closest('[data-cart-prod]');
  var productId = Number(productNode.getAttribute('data-cart-prod')); // Запрос к серверу
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default.a.ajax({
    ...
    success: function success(res) {
      ...
      productNode.remove(); // Удаляем этот же товар с других мест страницы

      [].slice.call(document.querySelectorAll("[data-cart-prod=\"".concat(productId, "\"]"))).forEach(function (item) {
        item.remove();
      }); // Если корзина пуста
...
$("body").on('submit', 'form.js-item', function () {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default.a.post(f.attr('action') + '?html=1', f.serialize(), function (response) {
    if (response.status == 'ok') {
      ...
      if (elems !== 0) {
        var check;
        jquery__WEBPACK_IMPORTED_MODULE_0___default.a.each(pList, function (i, v) {
          if ($(v).data('id') == pId) {
            check = false;
          }
        });
...
$(document).on('click', ".js-product-buy", function (e) {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default.a.post('/cart/add/', {
    html: 1,
    product_id: id,
    quantity: qty
  }, function (response) {
...
function recalculate() {
  var itemPriceElem = $('.js-result-price');
  var length = parseInt($('div[data-length-active]').attr('data-price'));
  if (isNaN(length)) {    length = 0;  }
  var color = parseInt($('div[data-color-active]').attr('data-price'));
  if (isNaN(color)) {    color = 0;  }
  var packing = parseInt($('div[data-packing-active]').attr('data-price'));

  if (isNaN(packing)) {
    packing = 0;
  }

  var itemPrice = parseInt(itemPriceElem.attr('data-price'));
  var qty = $('.qnt-toggle__val').val();
  var newBouqetPrice = (itemPrice + color + length) * qty + packing;
...
function wordForms(val) {
  ...
  $('.result-info__name').html(result); // $('.result-info__qty').hide().html('');
}
...
$(document).on('click', '.length-toggle', function () {
  ...
  if (_typeof(attr) !== ( true ? "undefined" : undefined) && attr !== false) { } else if (!attr) {
    $(this).siblings().removeAttr('data-length-active');
    $(this).attr('data-length-active', '');
    recalculate();
    var lengthValue = $(this).attr('data-action');
...
$(document).on('click', '.js-color-toggle', function () {
  ...
  if (_typeof(attr) !== ( true ? "undefined" : undefined) && attr !== false) {} else if (!attr) {
    ...
    var colorCount = $('.js-color-toggle[data-color-active]').attr('data-color-count');
    var qty = parseInt($('.qnt-toggle__val').val());
...
$(".js-header-cart-items").on('click', ".js-header-cart-product-delete", function () {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default.a.post('/cart/delete/', {
    ...
    if (response.data.count == 0) {
      ...
      $('.js-header-cart-message').show(); //   cart.append('<div class="header-cart-container__empty js-header-cart-message">Корзина пуста</div>');
...
function updateCartHeader(productId, qnt) {} // Homepage Circle-slider
